...
type SchedulesErrorPageProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function SchedulesErrorPage({
  error,
  reset,
}: SchedulesErrorPageProps) {
  return (
        ...
        <Card className="admin-reveal w-full overflow-hidden rounded-[30px] border-white/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.97),rgba(247,250,252,0.92))] shadow-soft">
          ...
          <CardHeader className="space-y-4 border-b border-slate-200/80 pb-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Badge variant="outline">Plannings</Badge>
            </div>
            <CardTitle className="text-2xl text-slate-950">
              La gestion des plannings n&apos;a pas pu s&apos;afficher
              correctement
            </CardTitle>
            <p className="max-w-2xl text-sm leading-6 text-slate-600">
              Une erreur inattendue a interrompu le chargement des données de
              planning. Aucun changement n&apos;a été appliqué.
            </p>
          </CardHeader>
