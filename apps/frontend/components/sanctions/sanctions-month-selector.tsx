...
type SanctionsMonthSelectorProps = {
  month: string;
};

export function SanctionsMonthSelector({ month }: SanctionsMonthSelectorProps) {
    ...
    <label className="flex w-full flex-col gap-2 sm:max-w-[220px]">
      ...
      <input
        ...
        onChange={(event) => {
          ...
          if (nextMonth) {
            router.replace(`/sanctions?month=${encodeURIComponent(nextMonth)}`);
          }
        }}
        type="month"
      />
